...
interface QueueState {
  ...
  loadQueueByTicket: (ticketId: string) => Promise<void>;
  refresh: (userId: string, ticketId?: string) => Promise<void>;
  clear: () => void;
}
...
export const useQueueStore = create<QueueState>((set, get) => ({
  ...
  /**
   * Carrega a fila por ID da passagem específica
   */
  loadQueueByTicket: async (ticketId: string) => {
    try {
      set({ isLoading: true, error: null });
      ...
      console.error('Erro ao carregar fila por ticket:', error);
      set({
        error: error.message || 'Erro ao carregar fila',
        isLoading: false,
      });
   ...
   */
  refresh: async (userId: string, ticketId?: string) => {
    if (ticketId) {
      await get().loadQueueByTicket(ticketId);
    } else {
